...
def mcts_rollout(tree_node, state_map, data, graph, score_func):
    ...
    if len(tree_node.children) == 0:
        ...
        for each_node in expand_nodes:
            # for each node, pruning it and get the remaining sub-graphs
            ...
            # check the state map and merge the same sub-graphs
            Find_same = False
...
def mcts(data, gnnNet, prototype, queue):
    ...
    return result_node.coalition, result_node.P, emb, mask_data
...
if __name__ == '__main__':
    ...
    #     data = dataset[i.item()]
    #     for j in range(10):
    #         coalition, _, _, _ = mcts(data, gnnNets, prototype_vectors[j])
    #         print(coalition)
    #         graph = to_networkx(data, to_undirected=True)
